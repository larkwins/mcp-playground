---
name: MCP Playground 全栈项目
overview: 搭建一个类 Postman 的 MCP Server 调试工具：前端 React+Vite+TS(Tailwind/shadcn) 输入远程 MCP Server URL 与自定义 Header 后连接，展示 tools/resources/prompts，可选中 tool 查看入参出参 schema 并填参调用查看结果；后端 Node+TS 用官方 @modelcontextprotocol/sdk 作为代理连接 MCP Server(自动探测 Streamable HTTP / SSE)规避浏览器 CORS，连接配置保存在 localStorage。
design:
  architecture:
    framework: react
    component: shadcn
...
todos:
  - id: scaffold
    content: 搭建 pnpm workspace monorepo，初始化 server(Express+TS)、web(Vite+React+TS+Tailwind+shadcn)、shared 三个包与共享类型
    status: completed
  - id: mcp-service
    content: Use [mcp:context7] 核对 SDK API，实现 mcpClient 封装(HTTP→SSE 自动探测、Header 透传)与会话池 sessionStore
    status: completed
    dependencies:
      - scaffold
  - id: server-api
    content: 实现 Express 路由 connect/disconnect/tools/resources/prompts/call/read/prompt，含统一错误规范化与脱敏日志
    status: completed
    dependencies:
      - mcp-service
  - id: connection-ui
    content: 实现 ConnectionBar 连接配置区(URL/传输选择/Header 编辑/状态指示)与 localStorage 持久化 hook
    status: completed
    dependencies:
      - scaffold
  - id: capability-ui
    content: 实现 CapabilityList 三分类列表(Tab+搜索)，接入后端能力查询并缓存
    status: completed
    dependencies:
      - connection-ui
      - server-api
  - id: tool-invoke-ui
    content: 实现 ToolDetailPanel(JSON Schema 入参表单+出参说明)与 ResultViewer 调用结果展示
    status: completed
    dependencies:
      - capability-ui
  - id: resource-prompt-ui
    content: 实现 ResourcePromptPanel 完成 Resource 读取与 Prompt 查看/渲染
    status: completed
    dependencies:
      - capability-ui
---
